...
const creditApi = axios.create({
  baseURL: "http://localhost:9009/api/v2",
  timeout: 15000,
...
export async function getCreditScore(userId: string | number): Promise<CreditScoreResponse | null> {
  try {
    ...
    if (error?.response?.status === 401) {
      console.error("❌ Unauthorized saat ambil credit score. Pastikan token cookie ada.")
    } else {
