...
export default function DraftPage() {
  ...
  const draftCustomers = getByStage("draft");
  
  // Filter customers based on search term
  const filteredCustomers = draftCustomers.filter(customer =>
  ...
  const handleReview = (customerId: string) => {
    // Move customer to review stage
    move(customerId, "review");
    // Navigate to review detail page
    router.push(`/admin/review/${customerId}`);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border p-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Draft Applications</h1>
            <p className="text-gray-600 mt-1">
              {filteredCustomers.length} pengajuan KPR menunggu review
            </p>
          </div>
          <div className="flex items-center space-x-4">
            <div className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-medium">
              {draftCustomers.length} Total Draft
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border">
        ...
        <Table
          ...
          columns={columns}
          emptyMessage="Tidak ada pengajuan draft yang ditemukan"
        />
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border p-6">
        <div className="flex items-center justify-between">
          <div className="text-sm text-gray-700">
            ...
            <span className="font-medium">{draftCustomers.length}</span> hasil
          </div>
